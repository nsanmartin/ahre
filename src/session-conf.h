...
static inline bool session_conf_show_forms(SessionConf sc[static 1]) {
    return sc->flags & SESSION_CONF_SHOW_FORM_JS;
}

static inline void _set_flag_(SessionConf sc[static 1], unsigned mask, bool value) {
    if (value) sc->flags |= mask;
    else sc->flags &= ~mask;
}

static inline void session_conf_show_forms_set(SessionConf sc[static 1], bool value) {
