...
static inline Err doc_cmd_hide(HtmlDoc d[static 1], const char* tags) {
    size_t ts = 0;
...
static inline Err doc_cmd_show(HtmlDoc d[static 1], const char* tags) {
    size_t ts = 0;
    Err err = htmldoc_tags_str_reduce_size_t(cstr_trim_space((char*)tags), &ts);
    if (!err) { *htmldoc_hide_tags(d) &= (~ts); }
    return err; 
...
static inline Err ed_print(Session s[static 1], TextBuf textbuf[static 1], Range range[static 1]) {
...
static inline Err _cmd_submit_ix(Session session[static 1], size_t ix) {
