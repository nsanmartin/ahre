...
int read_line_from_user(Session session[static 1]) {
    char* line = 0x0;
    line = readline("");
    ErrStr err = session->user_line_callback(session, line);
    if (err) {
        destroy(line);
        printf("error: %s.\n", err);
        return -1;
...
ErrStr cmd_set_url(Session session[static 1], const char* url) {
    puts("url");
    Str u;
...
ErrStr cmd_eval(Session session[static 1], const char* line) {
    const char* rest = 0x0;
...
ErrStr ed_eval(Session session[static 1], const char* line) {
    const char* rest = 0x0;
    if (!line) { return NULL; }
    Range range = {0};
    ...
    if ((rest = substr_match(line, "quit", 1)) && !*rest) { session->quit = true; return NULL;}
